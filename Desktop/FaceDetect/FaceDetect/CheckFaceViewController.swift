...
var checkFaceModel = CheckFaceModel()

class CheckFaceViewController: BaseViewController, AVCaptureVideoDataOutputSampleBufferDelegate {

    @IBOutlet weak var previewView: UIView!
    @IBOutlet weak var imgBackground: UIImageView!
    @IBOutlet weak var constraintTopBackground: NSLayoutConstraint!
    ...
    var checkface = ""
    var img_name = ""
    var isCallUnLock = false//check call api unlock
    var countdownHiddenViewInfo: Timer?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.constraintTopBackground.constant = 0
    }
    
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        self.trackingRequests?.removeAll()
        self.isCallUnLock = false
    }
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.2) {
                self.constraintTopBackground.constant = self.view.frame.height
                self.view.layoutIfNeeded()
            }
    ...
    func sendFaceImageToServer(imgFace: UIImage) {
        ...
        let url = "https://picture.vision-vietnam.com/public/aws/api?tab=es"
                            ...
                            upload.responseJSON { response in
                                ...
                                if (dic?["status"] as? String == "checkface_done") {
                                    if (dic?["data"] != nil) && dic?["data"] is Array<Dictionary<String, String>> {
                                        checkFaceModel = CheckFaceModel(dictionary: dic! as NSDictionary)
                                        self.callAPIUnLock()
    ...
    func sendResultToServer(isCorrect: String) {
        self.countdownHiddenViewInfo?.invalidate()
        print("Start send RESULT to server")
        let todosEndpoint: String = "https://picture.vision-vietnam.com/public/aws/api?tab=es&log_id=\(self.log_id)&name=\(self.name)&checkface=\(isCorrect)"
    ...
    //MARK: Call API UnLock
    func callAPIUnLock(){
        if self.isCallUnLock{
            return
        }
        services.requestAPIWithBody(success: { [weak self] (res) in
            self?.isCallUnLock = true
            self?.pushInfo()
        }) { [weak self] (err) in
    ...
    func pushInfo(){
        let vc = InfoFaceViewController()
        vc.itemUser = checkFaceModel
